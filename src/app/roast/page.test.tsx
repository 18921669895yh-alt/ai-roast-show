...
describe("RoastPage", () => {
  ...
  it("shows the host intro and accurate privacy disclosure", () => {
    render(<RoastPage />);
    expect(screen.getByText("把那条朋友圈交出来。我们只研究它为什么普通，却非要把自己剪成预告片。")).toBeVisible();
  ...
  it("requires text or image and defaults to familiar", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "开始锐评" }));
  ...
  it("posts exact text, image, level and mode then stores and previews useful result text before navigation", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "锐评先看这几句" })).toBeVisible();
    ...
    await user.click(screen.getByRole("button", { name: "查看完整锐评" }));
  ...
  it("retains all input after failure and retries", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "开始锐评" }));
    expect(await screen.findByRole("button", { name: "重新锐评" })).toBeVisible();
    ...
    expect(screen.getByRole("heading", { name: "锐评先看这几句" })).toBeVisible();
  });
  ...
  it("aborts an active request and returns to idle with inputs", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "开始锐评" })).toBeVisible());
  ...
  it("prevents double submission", async () => {
    ...
    const submit = screen.getByRole("button", { name: "开始锐评" });
